...
## for sacign results
sol_Ppv =  zeros(T, N_hs)
sol_E_bat = zeros(T, N_hs)
sol_Pch_bat = zeros(T, N_hs)
sol_Pdis_bat = zeros(T, N_hs)
sol_Pnet_house = zeros(T, N_hs)
sol_Pgrid_house_imp = zeros(T, N_hs)
sol_Pgrid_house_exp = zeros(T, N_hs)
sol_cost_house_step = zeros(T, N_hs)
sol_cost_house = zeros(N_hs,1)
...
for i = 1:N_hs
    ...
    @variable(m_opt, Pnet_house[1:T]) # the net kw of each house
    # battery
    @variable(m_opt, E_bat[1:T] )   # Battery energy
    @variable(m_opt, Pch_bat[1:T])  # charging power on grid side
    @variable(m_opt, Pdis_bat[1:T]) # discharging power on grid side
    # PV
    @variable(m_opt, Ppv[1:T] )   # PV power
    ...
    # constraints- balance
    @constraint(m_opt, pow_balan_limit[t=1:T], Pnet_house[t] == Pload_profile[t,i]
                                                                  - Pdis_bat[t] + Pch_bat[t] - Ppv[t]  )
    @constraint(m_opt, pow_net_limit[t=1:T], Pnet_house[t] == Pgrid_imp[t] - Pgrid_exp[t] )
    @constraint(m_opt, Pgrid_imp_limit[t=1:T], 0 <= Pgrid_imp[t] <= M_big )
    @constraint(m_opt, Pgrid_exp_limit[t=1:T], 0 <= Pgrid_exp[t] <= M_big )
    # constraints- battery
    @constraint(m_opt, bat_Pch_limit[t=1:T], 0 <= Pch_bat[t] <= Prate_bat[i] )
    @constraint(m_opt, bat_Pdis_limit[t=1:T], 0 <= Pdis_bat[t] <= Prate_bat[i] )

    @constraint(m_opt, bat_energy_limit[t=1:T], soc_min*Erate_bat[i] <= E_bat[t] <= soc_max*Erate_bat[i] )
    @constraint(m_opt, bat_end_limit[t=T], E_bat[t] == soc_ini*Erate_bat[i] );
    @constraint(m_opt, bat_balan_limit_1[t=1], E_bat[t] == soc_ini*Erate_bat[i] + (Pch_bat[t]*eta_bat - Pdis_bat[t]/eta_bat)*delta_t );
    @constraint(m_opt, bat_balan_limit_2[t=2:T], E_bat[t] == E_bat[t-1] + (Pch_bat[t]*eta_bat - Pdis_bat[t]/eta_bat)*delta_t );
    # contraints -pv
    @constraint(m_opt, pv_power_limit[t=1:T], 0 <= Ppv[t] <= Ppv_profile[t,i] )
    ...
    if sol_status == MOI.OPTIMAL
        ...
        sol_cost_each = JuMP.objective_value(m_opt)
        println("The objective function cost is \$", round(sol_cost_each; digits=3), ".\n");

        sol_Pgrid_each_imp = JuMP.value.(Pgrid_imp);
        sol_Pgrid_each_exp = JuMP.value.(Pgrid_exp);
        sol_Pnet_each = JuMP.value.(Pnet_house);
        sol_E_bat_each = JuMP.value.(E_bat);
        sol_Pch_bat_each = JuMP.value.(Pch_bat);
        sol_Pdis_bat_each = JuMP.value.(Pdis_bat);
        sol_Ppv_each = JuMP.value.(Ppv);

        sol_cost_each_step = sol_Pgrid_each_imp.*grid_price_imp*delta_t - sol_Pgrid_each_exp.*grid_price_exp*delta_t

        sol_Ppv[:,i] =  sol_Ppv_each
        sol_E_bat[:,i] = sol_E_bat_each
        sol_Pch_bat[:,i] = sol_Pch_bat_each
        sol_Pdis_bat[:,i] = sol_Pdis_bat_each
        sol_Pnet_house[:,i] = sol_Pnet_each
        sol_Pgrid_house_imp[:,i] = sol_Pgrid_each_imp
        sol_Pgrid_house_exp[:,i] = sol_Pgrid_each_exp
        sol_cost_house_step[:,i] = sol_cost_each_step
        sol_cost_house[i] = sol_cost_each
    end
end

sol_Pgrid_imp = vec( sum(sol_Pgrid_house_imp, dims=2) )
sol_Pgrid_exp = vec( sum(sol_Pgrid_house_exp, dims=2) )
sol_Pnet_pcc = sol_Pgrid_imp - sol_Pgrid_exp
sol_cost_step = sum(sol_cost_house_step,dims=2)
...
plot!(splt[1], vec_xaxis, sol_Pnet_house, label = ["House 1" "House 2" "House 3"], legend=:topleft, ylims=(-4,9) )
...
plot!(splt[2], vec_xaxis, sol_cost_house_step, label = ["House 1" "House 2" "House 3"], legend=:topleft, ylims=(-25,9*5) )
...
for i = 1:N_hs
    ...
    plot!(splt[i], vec_xaxis, sol_Pch_bat[:,i]-sol_Pdis_bat[:,i], label = string("Bat-ch @ H ",i), legend=:bottomleft)
    plot!(xlab="Hour", ylab="Power (kW)")
    plot!(splt[i], ylim = (-4, 4))
...
vec_objective_Value[1:N_hs] =  sol_cost_house;
